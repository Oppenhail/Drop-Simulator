...
interface RawDrop {
  ...
  function parseDropsFromHTML(html: string): BossDrops {
    ...
    const normal: RawDrop[] = [], gemRare: RawDrop[] = [], wilderness: RawDrop[] = [];
  ...
  function computeTooltip(q:number,p:string):string {
    if(p.includes('–')){
  ...
  async function renderAll(){
    const page=bossSelect.value; if(!page) return;
    if(page==='__add'){
      const input=prompt(
        "Paste a RSWiki URL, e.g. https://runescape.wiki/w/Woman"
      )?.trim();
      if(!input||!input.startsWith("https://runescape.wiki/w/")){
        alert("URL must begin with https://runescape.wiki/w/");
        bossSelect.value=""; return;
      }
    ...
    function makeSection(title:string, items:RawDrop[], offset:number){
      ...
      const h2=document.createElement('h2'); h2.textContent=title; container.append(h2);
      ...
      items.forEach((d,i)=>{
        let raw=d.rateParts[
          (d.rateParts.length>1&&slayerTaskToggle.checked)?1:0
        ].replace(/\u00A0/g,' ')
         .replace(/\s+/g,' ')
         .replace(/,/g,'')
         .replace(/^~/,'')
        ...
        if(pm) raw=`${Math.round(parseFloat(pm[1]))}%`;
        else {
          const om=raw.match(/^1\/(\d+(?:\.\d+)?)$/);
          if(om) raw=`1/${Math.ceil(parseFloat(om[1]))}`;
          else {
  ...
  function updateUI(){
    document.querySelectorAll('td.quantity-cell').forEach((el,idx)=>{
      const td=el as HTMLElement, q=currentCounts[idx];
  ...
  document.getElementById('autoKillToggle')!.addEventListener('change',e=>{
    ...
    if((e.target as HTMLInputElement).checked){
      ...
      autoKillTimer=window.setInterval(()=>{
        ...
        simulateOneKill(); updateUI();
      },20);
    }
  });
  document.getElementById('reset')!.addEventListener('click',()=>{
    if(autoKillTimer!==null){ clearInterval(autoKillTimer); autoKillTimer=null; }
    killsInput.value='0'; currentCounts=[]; document.getElementById('tablesContainer')!.innerHTML=''; sumEl.textContent='Total GE Value: 0 coins';
  });
