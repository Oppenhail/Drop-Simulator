...
const bossData: Record<string, BossDrops> = {};  // <--- Added missing declaration

const bossSelect = $('bossSelect') as HTMLSelectElement;
const killsInput = $('kills') as HTMLInputElement;
const sumEl = $('sumGE') as HTMLElement;
const tooltip = $('tooltip') as HTMLElement;
const slayerTaskToggle = $('slayerTaskToggle') as HTMLInputElement;
const slayerTaskContainer = $('slayerTaskContainer') as HTMLElement;
const wildernessToggle = $('wildernessToggle') as HTMLInputElement;
const wildernessToggleContainer = $('wildernessToggleContainer') as HTMLElement;
const luckToggle = $('luckToggle') as HTMLInputElement;
const simulateBtn = $('simulate') as HTMLElement;
const autoKillToggle = $('autoKillToggle') as HTMLInputElement;
const resetBtn = $('reset') as HTMLElement;
const tablesContainer = $('tablesContainer') as HTMLElement;
...
// ─── Parser Utilities ─────────────────────────────────────────────────────────
function normalizeRatePart(raw: string): string {
  let s = raw.replace(/\u00A0/g, ' ')
             .replace(/\s+/g, ' ')
             .replace(/,/g, '')
             .replace(/^~/, '')
  ...
  if (o) return `1/${Math.ceil(parseFloat(o[1]))}`;
  const m = s.match(/^(\d+)\s*[×x]\s*(\d+)\/(\d+)$/i);
  if (m) {
    const numer = parseInt(m[2], 10), denom = parseInt(m[3], 10);
    return `1/${Math.ceil(denom / numer)}`;
  }
  if (/always/i.test(s))    return 'Always';
  if (/very rare/i.test(s)) return '1/500';
  if (/uncommon/i.test(s))  return '1/30';
  if (/common/i.test(s))    return '1/3';
  if (/rare/i.test(s))      return '1/100';
  return s;
}

function parseRate(rt: string): number {
  const c = rt.replace(/\[.*?\]/g, '').replace(/,/g, '').trim().toLowerCase();
  if (c === 'always') return 1;
  const p = c.match(/^([\d.]+)%$/);
...
function parseGEPrice(s: string): PriceRange {
  const parts = s.split('–').map(str => parseInt(str.replace(/[, ]+/g, ''), 10) || 0);
  return { min: parts[0], max: parts[1] ?? parts[0] };
}

// ─── Fetch & Parse ────────────────────────────────────────────────────────────
async function fetchDrops(page: string): Promise<void> {
  if (bossData[page]) return;
...
function findDropsHeading(doc: Document): Element | null {
  return Array.from(doc.querySelectorAll('span.mw-headline'))
    .find(h => /drops?|loot/i.test(h.textContent || '')) || null;
}

function parseDropsFromHTML(html: string): BossDrops {
  const doc = new DOMParser().parseFromString(html, 'text/html');
  const normal: RawDrop[] = [];
  const gemRare: RawDrop[] = [];
  const wilderness: RawDrop[] = [];
  const heading = findDropsHeading(doc);
  if (!heading) return { normal, gemRare, wilderness };
  let node = heading.parentElement!.nextElementSibling;
  let cat: 'normal' | 'gemRare' | 'wilderness' = 'normal';

  while (node && node.tagName !== 'H2') {
    if (/^H[3-4]$/.test(node.tagName)) {
      const txt = node.textContent || '';
      if (/gem and rare drop table/i.test(txt))             cat = 'gemRare';
      else if (/wilderness (?:shared )?drop table/i.test(txt)) cat = 'wilderness';
      else                                                  cat = 'normal';
    }
...
function extractTable(tbl: HTMLTableElement, out: RawDrop[]): void {
  tbl.querySelectorAll('tr').forEach((row, i) => {
    ...
    if (!name) return;
    if (/^Wilderness shared loot table$/i.test(name)) return;
    if (/loot table|drop table/i.test(name)) return;
    ...
    let qtyPerDrop: number | undefined;
    if (/^Coins\b/i.test(name)) qtyPerDrop = 1;
...
function extractCharmsTable(tbl: HTMLTableElement, out: RawDrop[]): void {
  ...
  let dataRow: HTMLTableRowElement | undefined;
  for (let i = rows.indexOf(headerRow) + 1; i < rows.length; i++) {
    const tds = rows[i].querySelectorAll('td');
    if (tds.length >= charms.length) { dataRow = rows[i] as HTMLTableRowElement; break; }
  }
  if (!dataRow) return;
  ...
  charms.forEach((ch, i) => {
    ...
    if (m) raw = `${Math.ceil((+m[1] + +m[2]) / 2)}%`;
    raw = raw.replace(/^(\d+)\.\d+%$/, '$1%');
    if (/always/i.test(raw))         raw = 'Always';
    else if (/very rare/i.test(raw)) raw = '1/500';
    else if (/uncommon/i.test(raw))  raw = '1/30';
    else if (/common/i.test(raw))    raw = '1/3';
    else if (/rare/i.test(raw))      raw = '1/100';
...
class DropSimulator {
  ...
  private drops: RawDrop[] = [];
  private probs: number[] = [];
  private triskelionGroup: number[] = [];
  private triskelionCycle = 0;
  private killLoopId: number | null = null;

  loadDrops(drops: RawDrop[]): void {
    this.drops = drops;
    this.counts = drops.map(() => 0);
    this.probs = drops.map(d =>
      parseRate(d.rateParts.length > 1 && slayerTaskToggle.checked
                ? d.rateParts[1]
                : d.rateParts[0])
    );
  ...
  simulateOneKill(): void {
    ...
    this.probs.forEach((p, i) => {
      if (this.triskelionGroup.includes(i)) return;
      if (Math.random() < p) {
  ...
  simulateBatch(kills: number): void {
  ...
  startAutoKill(): void {
    if (this.killLoopId != null) return;
  ...
  getCounts(): number[] { return this.counts; }
  getDrops(): RawDrop[] { return this.drops; }
}

// ─── Tooltip & Value Helpers ─────────────────────────────────────────────────
function computeTooltip(d: RawDrop, q: number): string {
...
function computeValue(d: RawDrop, q: number): [number, number] {
  const { min, max } = parseGEPrice(d.gePrice);
  const perDrop = d.qtyPerDrop != null ? d.qtyPerDrop : (d.qtyMin ?? 1);
...
async function renderAll(): Promise<void> {
  ...
  if (!hasW) wildernessToggle.checked = false;
  ...
  if (!hasSl) slayerTaskToggle.checked = false;
...
function renderTables(normal: RawDrop[], wilderness: RawDrop[]): void {
  tablesContainer.innerHTML = "";

  function makeSection(title: string, items: RawDrop[], offset: number): void {
  ...
  document.querySelectorAll("td.quantity-cell").forEach(el => {
    ...
    td.onmousemove = (e: MouseEvent) => {
      const off = 12;
      const r = tooltip.getBoundingClientRect();
      let x = e.clientX + off;
      if (x + r.width > window.innerWidth) x = e.clientX - off - r.width;
...
function updateUI(): void {
  ...
  simulator.getDrops().forEach((d, i) => {
    const td = document.querySelectorAll("td.quantity-cell")[i] as HTMLElement;
    const q = counts[i];
...
autoKillToggle.addEventListener("change", (e) => {
  if ((e.target as HTMLInputElement).checked) simulator.startAutoKill();
  else simulator.stopAutoKill();
});
