...
interface RawDrop {
    ...
    gePrice:     string;
    icon?:       string;
    // for charms (and coins, if forced) a fixed drop quantity:
    qtyPerDrop?: number;
    // for normal drops, a range:
    qtyMin?:     number;
    qtyMax?:     number;
  }
  ...
  // ─── Globals ─────────────────────────────────────────────────────────────────
  const bossData: Record<string, BossDrops> = {};
  
  let currentCounts: number[]    = [];
  let activeDrops:    RawDrop[]  = [];
  let activeProbs:    number[]   = [];
  let autoKillTimer:  number|null= null;
  let triskelionGroup: number[]  = [];
  let triskelionCycle = 0;
  
  const bossSelect                = document.getElementById('bossSelect')        as HTMLSelectElement;
  const killsInput                = document.getElementById('kills')           as HTMLInputElement;
  const sumEl                     = document.getElementById('sumGE')!;
  const tooltip                   = document.getElementById('tooltip')!;
  const slayerTaskToggle          = document.getElementById('slayerTaskToggle')  as HTMLInputElement;
  const slayerTaskContainer       = document.getElementById('slayerTaskContainer')!;
  const wildernessToggle          = document.getElementById('wildernessToggle')  as HTMLInputElement;
  const wildernessToggleContainer = document.getElementById('wildernessToggleContainer')!;
  const luckToggle                = document.getElementById('luckToggle')        as HTMLInputElement;
  ...
  // ─── Fetch & Parse ────────────────────────────────────────────────────────────
  async function fetchDrops(page: string) {
    ...
    for (const cat of ['normal','wilderness'] as const) {
  ...
  // ─── Table Extractors ─────────────────────────────────────────────────────────
  function extractTable(tbl: Element, out: RawDrop[]) {
    tbl.querySelectorAll('tr').forEach((row,i) => {
      ...
      let qtyMin: number|undefined, qtyMax: number|undefined;
  ...
  function extractCharmsTable(tbl: HTMLTableElement, out: RawDrop[]) {
    ...
    let idx = rows.indexOf(headerRow)+1, dataRow: HTMLTableRowElement|undefined;
    while(idx<rows.length){
      const tds = rows[idx].querySelectorAll('td');
      if(tds.length>=charms.length){ dataRow = rows[idx] as any; break; }
      idx++;
    }
    if(!dataRow) return;
...
function computeTooltip(d: RawDrop, q: number): string {
  ...
  function simulateOneKill(){
    ...
    if (triskelionGroup.length) {
      ...
      if (Math.random() < p) {
        ...
        const qty   = d.qtyPerDrop ?? (
                        d.qtyMin!==undefined && d.qtyMax!==undefined
    ...
    activeProbs.forEach((p,i)=>{
      if (triskelionGroup.includes(i)) return;
      if(Math.random()<p){
  ...
  function prepareAndSimulate(base:RawDrop[]){
    currentCounts = base.map(()=>0);
    triskelionCycle = 0;
    const kills = +killsInput.value || 0;
    const probs = base.map(d=> parseRate(
      d.rateParts.length>1 && slayerTaskToggle.checked
        ? d.rateParts[1]
        : d.rateParts[0]
    ));
...
function computeValue(d: RawDrop, q: number): [number, number] {
  ...
  async function renderAll(){
    ...
    if(page==='__add'){
      ...
      if(!input || !input.startsWith("https://runescape.wiki/w/")){
        alert("URL must begin with https://runescape.wiki/w/"); bossSelect.value=""; return;
      }
    ...
    function makeSection(title: string, items: RawDrop[], offset: number) {
    ...
    makeSection('Wilderness drop table',
                wildernessToggle.checked ? wilderness : [],
                normal.length);
  ...
  document.getElementById('autoKillToggle')!.addEventListener('change',e=>{
    if(autoKillTimer!==null){ clearInterval(autoKillTimer); autoKillTimer=null; }
    if((e.target as HTMLInputElement).checked){
      if(!currentCounts.length) renderAll();
